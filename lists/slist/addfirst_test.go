...
func TestAddFirst_LLNode(t *testing.T) {
	...
	t.Run("Call AddFirst(SListNode<T>) several times", func(t *testing.T) {
		linkedList = New[int]()
		for i := 0; i < arraySize; i++ {
	...
	t.Run("Call AddFirst(SListNode<T>) several times remove all of the items", func(t *testing.T) {
		linkedList = New[int]()
		for i := 0; i < arraySize; i++ {
			linkedList.AddNodeFirst(NewNode(headItems[i]))
		}
...
func TestAddFirst_LLNode_Negative(t *testing.T) {
	...
	t.Run("Verify Node that already exists in another collection", func(t *testing.T) {
		...
		tempSList.AddItemLast(util.CreateRandInt(&seed))
		require.Panics(t, func() { linkedList.AddNodeFirst(tempSList.Last()) })
		initialItems_Tests(t, linkedList, items)
	})
}
