...
func TestForwardIterator(t *testing.T) {
	...
	t.Run("Iterator receives all values added to set", func(t *testing.T) {
		...
		require.ElementsMatch(t, setItems, iteratedItems)
	})

	t.Run("Iterator skips empty bucket", func(t *testing.T) {
		set := New[int]()

		set.AddRange(setItems)
		set.Remove(setItems[0])
		...
		require.ElementsMatch(t, setItems[1:], iteratedItems)
...
func TestForwardIteratorWithHashCollision(t *testing.T) {
	...
	t.Run("Iterator receives all values added to set", func(t *testing.T) {
		set := New(WithHasher(func(v int) uintptr { return uintptr(v % 4) }))
