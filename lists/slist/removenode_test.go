...
// 	seed := int64(21543)
// 	var items []int
// 	nodes := make([]*SListNode[int], arraySize*2)
// 	var currentNode *SListNode[int]
// 	var index int
...
func TestRemove_LLNode_Negative(t *testing.T) {
	...
	t.Run("Verify Node that already exists in another collection", func(t *testing.T) {
		...
		tempSList.AddItemLast(util.CreateRandInt(&seed))
		require.Panics(t, func() { linkedList.RemoveNode(tempSList.Last()) }) //"Err_98809ahied Node that already exists in another collection throws InvalidOperationException\n"
