...
func TestAddLast_LLNode(t *testing.T) {
	...
	t.Run("Call AddLast(DListNode<T>) several times", func(t *testing.T) {
		linkedList = New[int]()
		for i := 0; i < arraySize; i++ {
	...
	t.Run("Call AddLast(DListNode<T>) several times remove some of the items", func(t *testing.T) {
		linkedList = New[int]()
		for i := 0; i < arraySize; i++ {
			linkedList.AddNodeLast(NewNode(tailItems[i]))
		}
...
func TestAddLast_LLNode_Negative(t *testing.T) {
	...
	t.Run("Verify Node that already exists in another collection", func(t *testing.T) {
		...
		tempDList.AddItemLast(util.CreateRandInt(&seed))
		require.Panics(t, func() { linkedList.AddNodeLast(tempDList.Last()) })
		initialItems_Tests(t, linkedList, items)
	})
}
