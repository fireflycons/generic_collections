...
func TestEnumerable(t *testing.T) {
	...
	t.Run("All is true for all even numbers", func(t *testing.T) {
		sl = New[int]()
		sl.AddRange(evens)
		sl.All(func(i int) bool { return i%2 == 0 })
		require.True(t, sl.All(func(i int) bool { return i%2 == 0 }))
	})

	t.Run("All (even numbers) is false for mixed even and odd numbers", func(t *testing.T) {
		sl = New[int]()
		sl.AddRange(mixed)
		require.False(t, sl.All(func(i int) bool { return i%2 == 0 }))
	})

	t.Run("Any is true for even number in mixed even and odd numbers", func(t *testing.T) {
		sl = New[int]()
		sl.AddRange(mixed)
		require.True(t, sl.Any(func(i int) bool { return i%2 == 0 }))
	})
	...
	t.Run("Any is false for number not in input slice", func(t *testing.T) {
		sl = New[int]()
		sl.AddRange(mixed)
		require.False(t, sl.Any(func(i int) bool { return i > 1000 }))
	})
	...
	t.Run("ForEach applies func to all elements", func(t *testing.T) {
		...
		sl.AddRange(evens)
		sl.ForEach(func(e collections.Element[int]) {
	...
	t.Run("ForEach sets all even elements to zero", func(t *testing.T) {
		...
		require.Equal(t, expected, sl.ToSlice())
	})
	...
	t.Run("Select selects all values <= 6", func(t *testing.T) {
		...
		sl.AddRange(evens)
		s1 := sl.Select(func(i int) bool { return i <= 6 })
...
func TestMinMax(t *testing.T) {
	...
	t.Run("Min", func(t *testing.T) {
		sl = New[int]()
		sl.AddRange(setItems)
		require.Equal(t, min, sl.Min())
	})

	t.Run("Max", func(t *testing.T) {
		sl = New[int]()
		sl.AddRange(setItems)
		require.Equal(t, max, sl.Max())
	})
}
